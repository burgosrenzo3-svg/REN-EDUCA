(async function iniciarVerModulo() {
    ...
    const cursoIdUrl = parametrosUrl.get("curso");
    const moduloIdUrl = parametrosUrl.get("modulo");
    const esPreviaPorUrl = parametrosUrl.get("previa") === "1";
    ...
    if (!modoVistaDocente && usuario && usuario.rol === "docente" && esPreviaPorUrl && cursoIdUrl && moduloIdUrl) {
        esVistaPreviaRapida = true;
        const cursosDocente = await obtenerCursosDeDocente(usuario.id);
        curso = cursosDocente.find((c) => String(c.id) === String(cursoIdUrl));
        modulo = curso?.modulos?.find((m) => String(m.id) === String(moduloIdUrl));
    ...
    if (linkVolver) {
        linkVolver.href = esVistaPreviaRapida ? `modulos.html?curso=${cursoIdParaProgreso}` : "curso.html";
        linkVolver.textContent = esVistaPreviaRapida ? "Volver a modulos" : "Volver al curso";
    }

    if (!modulo) {
        const volverUrlSinModulo = esVistaPreviaRapida && cursoIdUrl ? `modulos.html?curso=${cursoIdUrl}` : (esVistaPreviaRapida ? "panel-docente.html" : "curso.html");
        document.body.innerHTML = `
            <main class="lesson-shell">
                <a class="back-link" href="${volverUrlSinModulo}">Volver</a>
    ...
    function cargarModulo() {
        ...
        videoContainer.innerHTML = contenido?.video
            ? crearRecursoVideo(contenido.video)
    ...
    /* Si el video es de YouTube o Vimeo, lo embebe directo en la
       pagina (asi el estudiante no tiene que salir del sitio).
       Si es un archivo de video normal (subido a Supabase o un
       enlace .mp4/.webm directo), usa el reproductor nativo. */
    function crearRecursoVideo(url) {
            ...
            return `
                <article class="resource-card">
                    ...
                    <div class="video-embed">
                        <iframe src="https://www.youtube.com/embed/${idYoutube}" title="Video de apoyo" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen loading="lazy"></iframe>
                    </div>
                </article>
            `;
        }
        ...
        if (idVimeo) {
            return `
                <article class="resource-card">
                    <h2>Video de apoyo</h2>
                    <div class="video-embed">
                        <iframe src="https://player.vimeo.com/video/${idVimeo}" title="Video de apoyo" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen loading="lazy"></iframe>
                    </div>
                </article>
            `;
        }
        ...
        return `
            <article class="resource-card">
                <h2>Video de apoyo</h2>
                <video class="video-nativo" src="${escapeHTML(url)}" controls preload="metadata"></video>
            </article>
        `;
    }
