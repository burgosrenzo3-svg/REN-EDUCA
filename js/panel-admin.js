...
async function construirPanelAdmin() {
    ...
    conectarVistaPreviaEditarCursoImagen();
    ...
    function abrirEditarCurso(curso) {
        ...
        if (curso.imagen) {
            ...
            if (ayudaEditarCursoImagen) ayudaEditarCursoImagen.textContent = "Enlace cargado. Cambialo para reemplazarlo.";
        } else {
            if (previewEditarCursoImagen) previewEditarCursoImagen.style.display = "none";
            if (ayudaEditarCursoImagen) ayudaEditarCursoImagen.textContent = "Pega el enlace de la imagen.";
        }
    ...
    /* Muestra la imagen del curso en vivo a medida que el admin
       pega el enlace, para confirmar que es la imagen correcta
       antes de guardar. */
    function conectarVistaPreviaEditarCursoImagen() {
        const inputImagen = document.getElementById("editarCursoImagen");
        const preview = document.getElementById("previewEditarCursoImagen");
...
document.addEventListener("DOMContentLoaded", () => {
    construirPanelAdmin().catch((error) => {
        console.error("Error al cargar el panel de administrador:", error);
        const contenedor = document.querySelector(".app-shell") || document.body;
        contenedor.innerHTML = `
            <div class="empty-state" style="margin:40px auto;max-width:480px;text-align:center;">
