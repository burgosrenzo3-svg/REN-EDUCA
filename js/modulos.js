(async function iniciarModulos() {
    ...
    let curso = cursos.find((c) => String(c.id) === String(cursoId));
    ...
    function contenidoModulo(index) {
        const modulo = curso.modulos[index];
        window.location.href = `contenido-modulo.html?curso=${curso.id}&modulo=${modulo.id}`;
    }

    function quizModulo(index) {
        const modulo = curso.modulos[index];
        window.location.href = `quiz-editor.html?curso=${curso.id}&modulo=${modulo.id}`;
    }

    function verModulo(index) {
        const modulo = curso.modulos[index];
        window.location.href = `ver-modulo.html?curso=${curso.id}&modulo=${modulo.id}&previa=1`;
    }
