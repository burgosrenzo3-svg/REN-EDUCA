...
(async function iniciarCatalogoIndex() {
    ...
    let cursos = [];
    try {
        cursos = typeof obtenerCursosPublicosResumen === "function"
            ? await obtenerCursosPublicosResumen()
            : [];
    } catch (error) {
        console.error("Error al cargar el catalogo publico:", error);
        contenedor.innerHTML = `<p class="catalogo-vacio">No se pudo cargar el catalogo. Intenta recargar la pagina.</p>`;
        return;
    }
