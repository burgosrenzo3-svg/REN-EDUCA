(async function iniciarCursosDocente() {
    ...
    conectarVistaPreviaImagenCurso();
    ...
    try {
        ...
        cursosContainer.innerHTML = `
            <div class="empty-state">
                <h3>No se pudieron cargar tus cursos</h3>
    ...
    function editarCursoLocal(index) {
        ...
        if (curso.imagen) {
            ...
            if (ayudaImagenCurso) ayudaImagenCurso.textContent = "Enlace cargado. Cambialo para reemplazarlo.";
        } else {
            if (previewImagenCurso) previewImagenCurso.style.display = "none";
            if (ayudaImagenCurso) ayudaImagenCurso.textContent = "Pega el enlace de la imagen.";
        }
    ...
    function gestionarModulos(index) {
        const curso = cursos[index];
        window.location.href = `modulos.html?curso=${curso.id}`;
    }
    ...
    /* Muestra la imagen del curso en vivo a medida que el docente
       pega el enlace, para confirmar que es la imagen correcta
       antes de guardar. */
    function conectarVistaPreviaImagenCurso() {
        const inputImagen = document.getElementById("imagenCurso");
        const preview = document.getElementById("previewImagenCurso");
