/* =======================================================
   ...
   desde cualquier dispositivo con su mismo correo y clave.
...
function mapearErrorSupabase(error) {
    ...
    if (mensaje.includes("Invalid login credentials")) return "Correo o contrasena incorrectos.";
    if (mensaje.includes("already registered") || mensaje.includes("already been registered")) return "Este correo ya esta registrado.";
    if (mensaje.includes("Password should be at least")) return "La contrasena debe tener al menos 6 caracteres.";
    if (mensaje.includes("For security purposes") || mensaje.includes("rate limit") || mensaje.includes("you can only request this")) return "Ya enviamos un enlace hace poco. Espera un momento antes de pedir otro.";
...
/* ---------- Registro / login / logout (sin contraseña, con enlace al correo) ----------
   REN EDUCA ya no usa contraseñas para estudiantes y docentes.
   En vez de eso, Supabase envia un "enlace magico" al correo de
   la persona. Al hacer click en ese enlace desde su bandeja de
   entrada, el navegador vuelve a la pagina ya con la sesion
   iniciada (Supabase la detecta automaticamente gracias a
   "detectSessionInUrl: true" en supabase-client.js).

   El mismo metodo (signInWithOtp) sirve tanto para CREAR cuenta
   (la primera vez que ese correo se usa) como para INICIAR SESION
   (las veces siguientes), exactamente como "Continuar con Google":
   un solo paso, un solo correo. */

/* Envia el enlace de acceso/registro al correo indicado.
   Si nombre/rol vienen informados (formulario de "Crear cuenta"),
   se guardan como metadata: el trigger de la base de datos los
   usara para crear el perfil la primera vez que entre por el enlace.
   Devuelve { ok, error } */
async function enviarEnlaceAcceso({ correo, nombre, rol }) {
    ...
    if (nombre && rol) {
        opciones.data = { nombre: nombre.trim(), rol };
    }
...
/* Si el usuario cierra sesion, inicia sesion en OTRA pestaña, o
   acaba de entrar por el enlace magico de su correo, esta pestaña
   se mantiene sincronizada. El evento "ren:sesionActualizada" le
   avisa a main.js que debe refrescar el header y, si corresponde,
